...
if __name__ == '__main__':
    ...
    if os.path.exists(output_file):
        with open(output_file, 'r') as f:
            existed_ids = [json.loads(_)['image'] for _ in f]
        print('{} test existed'.format(len(existed_ids)))
        eval_dataset = [data for data in eval_dataset if data['image_id'] not in existed_ids]
    
    for data in tqdm.tqdm(eval_dataset):
        image_id = data['image_id']
        image_path = os.path.join(src_dir, '{}.jpg'.format(image_id))
        ...
        try:
            ...
            with open(output_file, 'a') as f:
                f.write(json.dumps({'image': image_id, 'question': question, 'response': response, 'answer': answer, 'full_answer': full_answer}))
                f.write('\n')
